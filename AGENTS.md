...
- `internal/ui/` centralizes all terminal UI components, `lipgloss` styling, and layout formatting. Command packages (`cmd/` and `internal/purrCommands/`) must strictly invoke exported helpers from `ui` rather than defining raw styles or instantiating `lipgloss` directly. Always use the standard output formatters: `ui.Successf` for success, `ui.Errorf` for errors, `ui.Warningf` for warnings, and `ui.Infof` for general information (as well as `ui.Hintf` for actionable suggestions). Only use `ui.Errorf` when execution is at a terminal stage or if the issue points to a dangerous state/data corruption. For non-critical issues, use `ui.Warningf` instead.
...
- Command repeat policy: repeated `init` requires interactive confirmation before preserving and repairing repository scaffolding; `commit` rejects unchanged trees; `add` and `config` are intentionally repeatable mutations; `ls` and `log` are repeatable inspections
